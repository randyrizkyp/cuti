...
class KassubagController extends Controller
{
   ...
   {
      $pegawai = Pegawai::where('kode_pd', session('kode_pd'))->where('statuspegawai', 'aktif')->get();
   ...
   function insertberkas(Request $request)
   {        
      if($request->file){
         ...
         Berkaspensiun::where('nip', $nip)->update($data);

         return redirect()->back()->with('success', 'Dokumen Berhasil Diupload');
      }  
   }

   function konfirmberkas(Request $request)
   {                        
      $nip = $request->nip;
      ...
      return redirect('/pengajuanpensiun')->with('success','Dokumen Berhasil Diajukan');
   }
   ...
   function tambahanak(Request $request)
   {
      $this->validate($request, [
   ...
   function ubahanak(Request $request)
   {
      $this->validate($request, [
         'nik'         => 'required|string|max:20',
         'nama'        => 'required|string|max:150',
   ...
   function tambahpasangan(Request $request)
   {
      $this->validate($request, [
         'nik'         => 'required|string|max:20',
         'nama'        => 'required|string|max:150',
   ...
   function ubahpasangan(Request $request)
   {
      $this->validate($request, [
         'nik'         => 'required|string|max:20',
         'nama'        => 'required|string|max:150',
